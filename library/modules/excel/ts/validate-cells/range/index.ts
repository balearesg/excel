...
export function validateRange({ cells, workbook }: IParamsValidateRange) {
	let errors = [];
	const isValidCellRange: boolean =
		!!cells && !!Array.isArray(cells) && !!cells.length;
	...
	try {
		for (const range of cells) {
			...
			for (const item of items) {
				const { startRow, endRow, startCol, endCol, type, regex } =
					item;
				const validated = validateValues({
					validate: {
						startRow: 'number',
				...
				for (let rowNum = startRow; rowNum <= endRow; rowNum++) {
					for (let colNum = startCol; colNum <= endCol; colNum++) {
						...
						if (value === undefined) {
							errors.push(
								`invalid range of cell for startRow: ${rowNum}, endRow: ${endRow}, startCol: ${startCol} and endCol ${endCol} this not exist`
							);
						}
						...
						if (!!regex && !type) {
							...
							if (!validate)
								errors.push(
									`Invalid data in row ${rowNum}, column ${colNum}: the value ${value} does not comply with the regex ${regex}`
								);
						}
						if (type && !dataTypeValidations[type]) {
							errors.push(
								`Invalid type in rangeValidation: ${type}`
							);
						}
						...
						if (
							type &&
							value !== undefined &&
							!dataTypeValidations[type](value, regex)
						) {
							errors.push(
								`Invalid data in row ${rowNum}, column ${colNum}: ${
									errorMsgs[type]
								} ${
									!!regex
										? `and comply with the regex ${regex}`
										: ''
								}`
							);
						}
